...
def minus_operator_handler(formula):
    """处理一些特殊的减号运算"""
    minus_operators = re.split("-", formula)    # 以匹配到的字符当做列表分隔符
    calc_list = re.findall("[0-9]", formula)    # 把所有匹配到的字符放到以列表中的元素返回
    if minus_operators[0] == '':   # 第一个值为负号
...
def compute(formula):
    """这里的计算是去除括号的"""
    formula = formula.strip("()")   # 去除外面的括号
    formula = remove_duplicates(formula)   # 去除重复的+-号
...
def calc(formula):
    ...
    while parenthesise_flag:
        ...
        if m:
            ...
            parenthesise_flag = False   # 代表公式里的括号已经都被剥除了
...
[-8.0] []
[-40/5]运算结果: -8.0
先算括号里的值： (9-2*5/3 + 7 /3*99/4*2998 +10 * 568/14 )
...
先算括号里的值： (60-30 +-8.0 * 173545.88095238098)
[8.0 * 173545.88095238098]运算结果= 1388367.0476190478
...
['16', 6.0] ['-']
[16-3*2]运算结果: 10.0
先算括号里的值： ( -1388337.0476190478 - -12.0/ 10.0 )
[12.0/ 10.0 ]运算结果= 1.2
['-1388337.0476190478 ', 1.2] ['+']
[ -1388337.0476190478 +12.0/ 10.0 ]运算结果: -1388335.8476190479
----没括号了...---
[ 2 * -1388335.8476190479]运算结果= -2776671.6952380957
